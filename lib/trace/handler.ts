...
import { Opik, Span, Trace, logger, SpanType, OpikSpanType, OpikConfig } from 'opik';
import {
  extractCallArgs,
...
export class NextLevelOpikCallbackHandler extends BaseCallbackHandler implements BaseCallbackHandlerInput {
  name = 'NextLevel-OpikCallbackHandler';
  ...
  // @> Keep reference of injected parent
  private parentRef: Trace | Span | null | undefined;
  ...
  constructor(options?: Partial<OpikCallbackHandlerOptions>) {
    ...
    // @> Properly initiate the state with the injected parent
    if (options?.parent) {
      this.parentRef = options?.parent ?? null;
      this.tracerMap.set(options.parent.data.id, options.parent);
    }
  }

  // @> Use to resolve the parent id
  private resolveParentId(parentRunId?: string): string | undefined {
    return parentRunId ?? this.parentRef?.data.id;
  }

  private startTracing({ runId, parentRunId, name, input, tags, metadata, type }: StartTracingArgs) {
  ...
  private endTracing({ runId, output, error, tags, usage, metadata }: EndTracingArgs) {
    let errorInfo;
    if (error) {
  ...
  async handleChatModelStart(
    ...
    logger.debug(`handleChatModelStart runId - ${runId}, parentRunId ${parentRunId}`);
    this.startTracing({
      runId,
      // @> Resolve parent id - repeated across handler
      parentRunId: this.resolveParentId(parentRunId),
  ...
  async handleLLMError(error: Error, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
  ...
  async handleLLMEnd(output: LLMResult | ChatResult, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
  ...
  async handleChainStart(
    ...
    logger.debug(`handleChainStart runId - ${runId}, parentRunId ${parentRunId}`);
  ...
  async handleChainError(error: Error, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
    logger.debug(`handleChainError runId - ${runId}, parentRunId ${parentRunId}`);
    this.endTracing({
      runId,
      tags,
      error
  ...
  async handleChainEnd(output: ChainValues, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
  ...
  async handleToolStart(
    ...
    logger.debug(`handleToolStart runId - ${runId}, parentRunId ${parentRunId}`);
    this.startTracing({
      runId,
      parentRunId: this.resolveParentId(parentRunId),
  ...
  async handleToolError(error: Error, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
    logger.debug(`handleToolError runId - ${runId}, parentRunId ${parentRunId}`);
    this.endTracing({
      runId,
      tags,
      error
  ...
  async handleToolEnd(output: unknown, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
  ...
  async handleAgentAction(action: AgentAction, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
    logger.debug(`handleAgentAction runId - ${runId}, parentRunId ${parentRunId}`);
    this.startTracing({
      runId,
      parentRunId: this.resolveParentId(parentRunId),
      name: action.tool,
  ...
  async handleAgentEnd(action: AgentFinish, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
  ...
  async handleRetrieverStart(
    ...
    logger.debug(`handleRetrieverStart runId - ${runId}, parentRunId ${parentRunId}`);
    this.startTracing({
      runId,
      parentRunId: this.resolveParentId(parentRunId),
  ...
  async handleRetrieverEnd(documents: unknown[], runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
    logger.debug(`handleRetrieverEnd runId - ${runId}, parentRunId ${parentRunId}`);
    this.endTracing({
      runId,
      output: { documents },
      tags
  ...
  async handleRetrieverError(error: Error, runId: string, parentRunId?: string, tags?: string[]): Promise<void> {
    logger.debug(`handleRetrieverError runId - ${runId}, parentRunId ${parentRunId}`);
    this.endTracing({
      runId,
      error,
      tags

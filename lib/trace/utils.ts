...
import { ContentContainer, KwargsContainer, MessageContainer, ValueContainer } from './types';

const isObject = (value: unknown): value is Record<string, unknown> => typeof value === 'object' && value !== null;

const hasContent = (obj: unknown): obj is ContentContainer & Record<string, unknown> => isObject(obj) && 'content' in obj;

const hasMessages = (obj: unknown): obj is MessageContainer & Record<string, unknown> => isObject(obj) && 'messages' in obj;

const hasValue = (obj: unknown): obj is ValueContainer & Record<string, unknown> => isObject(obj) && 'value' in obj;

const hasKwargs = (obj: unknown): obj is KwargsContainer & Record<string, unknown> => isObject(obj) && 'kwargs' in obj;

export const pick = (...values: unknown[]) => values.find((value) => value !== undefined && value !== null);
...
export const getMessageContent = (message: BaseMessage): Record<string, unknown> => {
...
export const parseGeneration = (generation: Generation | ChatGeneration): unknown => {
...
export const outputFromGenerations = (input: Generation[][] | ChatGeneration[]) => {
  const generations = input.flatMap((batch) => {
    return Array.isArray(batch) ? batch.map(parseGeneration) : parseGeneration(batch);
  });
...
export const extractCallArgs = (
  ...
  const args = cleanObject({
    ...
    max_tokens: pick(invocationParams?.max_tokens, invocationParams?.maxOutputTokens),
...
const parseChainValue = (output: unknown): Record<string, unknown> => {
  ...
  if (hasContent(output) && output.content !== undefined) {
    ...
    return isObject(content) ? (content as Record<string, unknown>) : { value: content };
  }

  if (hasMessages(output) && output.messages !== undefined) {
    const messages = Array.isArray(output.messages) ? output.messages.map(parseChainValue) : [parseChainValue(output.messages)];
    return { messages };
  }
  ...
  return Object.fromEntries(Object.entries(output).map(([key, value]) => [key, parseChainValue(value)]));
};

export const outputFromChainValues = (output: unknown): Record<string, unknown> => {
  try {
...
export const inputFromChainValues = (inputs: ChainValues): Record<string, unknown> => {
  try {
